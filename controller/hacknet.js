...
export async function main(ns) {
	...
	// var horizon = 5.0; // don't upgrade if roi > horizon hours
	var horizon = 0.5; // don't upgrade if roi > horizon hours
	var logs = [];
	...
	async function buyCorpFunds() {
		var upg = "Sell for Corporation Funds"
		hn.getHashUpgradeLevel(upg);
		hn.spendHashes(upg, "", 1);
	}

	async function buyCorpResearch() {
		var upg = "Exchange for Corporation Research"
		hn.getHashUpgradeLevel(upg);
		hn.spendHashes(upg, "", 1);
	}

	async function upgradeNodes(do_log, do_buy) {
		var bestServer = null;
	...
	for (var j = 0; j < hn.numNodes(); j++) {
		await upgradeNodes(false, true);
		await ns.sleep(10);
	}

	await upgradeNodes(true, false);
	await sellHashes();
	// await buyCorpResearch();
	// await buyCorpFunds();
